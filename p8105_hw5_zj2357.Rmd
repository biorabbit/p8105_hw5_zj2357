...
For u $\in$ {1,2,3,4,5,6}, $\hat{u}_{rej} \geq \hat{u}_{all}$. However, the gap $\hat{u}_{rej}$ - $\hat{u}_{all}$ gradually decreases with the increase of $u$. Because when $u \geq 5$, nearly all samples are rejected, $\hat{u}_{rej} \approx \hat{u}_{all}$ under such conditions.
