...
For u $\in$ {1,2,3,4,5,6}, `u_rej` is highher than `u_all`. However, the
gap `u_rej` - `u_all` gradually decreases with the increase of $u$.
Because when $u \geq 5$, nearly all samples are rejected, `u_rej`
$\approx$ `u_all` under such conditions.
